...
echo '* db creation' && \
    (     echo 'create role admin;';
	  echo 'create role client;';
	  echo 'create role anon;') | ./psql-admin.sh && \
		  ...
		  (cd sql/schema ;
		   ...
		   ( ( [[ ! -z "$RDS" ]] && echo ../pgjwt.sql || echo '../pgjwt-nords.sql' ) ; echo 00 ; egrep -v '^\#' order.txt | grep -v 'pgjwt' ) | xargs cat | \
		      ...
		      echo '* CLI Admin' && \
		      $DIR/psql.sh -c "insert into basic_auth.users(email,pass,role,validated,approved) values('$POSTGREST_CLI_LOGIN','$POSTGREST_CLI_PASS','admin',now(),now())"
